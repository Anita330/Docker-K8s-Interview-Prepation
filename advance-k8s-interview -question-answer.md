...
The API Server acts as the central hub and the only component that talks directly to etcd. Consistency
...
**iptables mode:** Kube-proxy watches for Service and Endpoint updates and creates individual
...
The Horizontal Pod Autoscaler controller runs a control loop (default 15s). It queries the Metrics Server
...
**Pod Networking:** Every pod gets a unique, ephemeral IP address. Pods communicate directly via
...
**Service LoadBalancer (L4):** Asks the Cloud Provider (AWS/GCP/Azure) to provision a
...
A Deployment manages updates by creating a new **ReplicaSet** while scaling down the old one. This
...
When a request hits the API Server, the RBAC Authorizer checks:
...
**In Transit:** Kubernetes uses **TLS** (HTTPS) for all communication between the User (kubectl), the
...
The Kubelet does not speak directly to Docker or containerd. It uses the **CRI (Container Runtime
Interface)**.
...
Static Pods are managed directly by the **Kubelet** on a specific node, bypassing the API Server and
...
Kubernetes GC cleans up unused resources (like terminated pods, unused images, or orphaned
...
If a container tries to consume more RAM than its defined `limit`, the Linux Kernel's **OOM (Out of
...
To achieve HA, you need to eliminate single points of failure:
